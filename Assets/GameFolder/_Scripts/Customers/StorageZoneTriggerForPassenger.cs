...
using UnityEngine;
using SKC.AIF.Storage.Trigger;
using SKC.AIF.Storage;

namespace SKC.AIF.Customers
{
    public class StorageZoneTriggerForPassenger : MonoBehaviour
    {
		...
		{
			if (other.TryGetComponent(out Storage.Storage inventory) && other.TryGetComponent(out CustomerBase customer))
			{
		...
		{
			if (other.TryGetComponent(out Storage.Storage inventory))
			{
		...
		IEnumerator Co_Collect(Storage.Storage inventory)
		{
			while (true)
			{
